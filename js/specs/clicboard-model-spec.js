...
describe('MapSwift.ClipboardModel', function () {
	'use strict';
	var underTest, storage, alertModel, resourceHandler;
	beforeEach(function () {
		storage = jasmine.createSpyObj('storage', ['setItem', 'getItem']);
		alertModel = jasmine.createSpyObj('alert', ['send']);
		...
		underTest = new MapSwift.ClipboardModel(storage, 'key-1', alertModel, resourceHandler);
	});
	describe('put', function () {
		...
		it('shows an alert when storage runs out of space', function () {
			...
			underTest.put('23456789');
			expect(alertModel.send).toHaveBeenCalledWith('error', 'clipboard', 'insufficient-space');
		});
